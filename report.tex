...
\begin{figure}[H]
    ...
    \node [database, below=1.5cm of server] (db) {H2 DB\\(Data)};
    \node [external, right=2.5cm of server] (llm) {Ollama\\(LLM)};
...
\begin{itemize}
    \item \textbf{Tecnologie:} Java 21, Spring Boot 3.5.8, Spring Security, JPA/Hibernate, Lombok.
    \item \textbf{Database:} H2 Database (In-Memory) per la persistenza veloce dei dati durante la simulazione.
    \item \textbf{Integrazione AI:} Modulo di comunicazione HTTP con \textbf{Ollama} (Llama 3.2 / Mistral) per l'elaborazione del linguaggio naturale.
\end{itemize}
...
Il componente centrale che exposifica tutti gli endpoint di business logic. Riceve richieste HTTP da client autenticati e orchestrata la business logic.
\begin{itemize}
    \item \textbf{Responsabilità:} Coordinare le operazioni di filtraggio, aggregazione, calcolo e analisi dei dati energetici.
    \item \textbf{Endpoints Principali:}
    \begin{enumerate}
        \item \texttt{/daily-report-session}: Aggrega dati per giorno e calcola metriche (produzione totale, temperatura media, status).
        \item \texttt{/batch-suggestions}: Esegue molteplici analisi in parallelo (best/worst day, anomalie).
        \item \texttt{/wind-impact}: Analizza l'impatto della velocità del vento sulla produzione.
        \item \texttt{/forecast}: Stima la produzione futura basata su medie storiche.
        \item \texttt{/financial-report}: Calcola ROI, valore economico e CO\textsubscript{2} risparmiata.
        \item \texttt{/peak-hours}: Identifica l'ora di massima produzione.
        \item \texttt{/smart-analysis}: Delegamittelfatta alla LlmService per analisi AI.
    \end{enumerate}
\end{itemize}

\paragraph{RenewableRepository}
Interfaccia JPA che incapsula le operazioni di query al database H2. Offre metodi per filtrare dati per range temporale e recuperare tutti i dati.
\begin{itemize}
...
Servizio dedicato all'integrazione con il modello di linguaggio Ollama. Costruisce un prompt strutturato e invia una richiesta HTTP al server Ollama locale.
\begin{itemize}
    \item \textbf{Metodo Principale:} \texttt{askAi(String promptData)}
    \item \textbf{Responsabilità:}
    \begin{enumerate}
        \item Arricchisce il prompt grezzo con un contesto professionale ("Sei un ingegnere energetico esperto...").
        \item Crea una richiesta HTTP POST verso \texttt{http://localhost:11434/api/generate}.
        \item Serializza i dati in JSON includendo il modello, il prompt, e flag di configurazione.
        \item Attende la risposta dal modello LLM e la restituisce al caller.
    \end{enumerate}
    \item \textbf{Gestione Errori:} Se Ollama non è attivo, ritorna un messaggio di errore user-friendly.
\end{itemize}

\paragraph{UserSession}
Mantiene il contesto di sessione dell'utente corrente, incluso il filtro temporale impostato.
\begin{itemize}
    ...
    \item \textbf{Nota:} È uno \textbf{Scoped Bean} di Spring, quindi ogni sessione HTTP ottiene un'istanza separata.
\end{itemize}
...
\begin{lstlisting}[language=Java, caption=Aggregazione dati lato backend - Remote Facade]
...
public List<DailyReportDTO> getDailyReportsWithSession() {
    ...
    // Filtro per range temporale (se attivo)
    if (userSession.isFilterActive()) {
        allData = allData.stream()
            .filter(d -> d.getDateTime().toLocalDate()
                    .isAfter(userSession.getStart())
                && d.getDateTime().toLocalDate()
                    .isBefore(userSession.getEnd()))
            .collect(Collectors.toList());
    }
    
    // Aggregazione per giorno
    Map<LocalDate, List<RenewableData>> groupedByDay = 
        allData.stream()
    ...
    return groupedByDay.entrySet().stream()
        .map(entry -> {
            ...
            double totalProd = dailyData.stream()
            ...
            String status = determineStatus(totalProd);
            
            return new DailyReportDTO(date, totalProd, avgTemp, status);
        })
...
L'integrazione con un sistema di generazione di linguaggio naturale (LLM) richiede adattare il formato dei dati energetici a quello atteso dal modello AI. Inoltre, il protocollo di comunicazione con Ollama (HTTP REST) deve essere incapsulato in modo trasparente dal resto dell'applicazione.

\subsection{Soluzione: LlmService come Adapter}
Il servizio \texttt{LlmService} funge da adapter tra il dominio energetico (EnergyController) e il dominio AI (Ollama).

\begin{lstlisting}[language=Java, caption=LlmService - Orchestrazione richiesta LLM]
@Service
public class LlmService {
    ...
    public String askAi(String promptData) {
        try {
            ...
            String fullPrompt = 
                "Sei un ingegnere energetico esperto. "
                + "Analizza questi dati brevemente "
                + "e dai un consiglio tecnico in italiano:\n" 
                + promptData;
            
            // 2. Crea il body della richiesta
            Map<String, Object> requestBody = new HashMap<>();
            requestBody.put("model", MODEL_NAME);
            requestBody.put("prompt", fullPrompt);
            requestBody.put("stream", false);
            
            // 3. Configura gli header HTTP
            ...
            // 5. Estrai e ritorna la risposta
            ...
            e.printStackTrace();
            return "Errore: Ollama non raggiungibile. "
                + "Esegui 'ollama serve'.";
        }
    }
}
...
Quando il client richiede l'analisi AI tramite l'opzione 9:

\begin{enumerate}
    \item \textbf{Client Istanza:} Utente seleziona opzione 9 e specifica un range data (es. 2017-06-01 a 2017-06-30).
...
Authorization: Bearer <jwt_token>
    ...
    \item \textbf{Elaborazione Backend:}
    \begin{lstlisting}[language=Java]
...
public String getSmartAnalysis(
    ...
    // Estrai dati dal range (max 15 record)
    ...
    if (start != null && end != null) {
        ...
        dataToAnalyze = repository.findByDateTimeBetween(
            startDate, 
            endDate
        );
    } else {
        dataToAnalyze = repository.findAll().stream()
            .sorted((a, b) -> b.getDateTime()
                    .compareTo(a.getDateTime()))
            .limit(3)  // Default: ultimi 3 record
            .collect(Collectors.toList());
    }
    
    // Limita a 15 record per non sovracaricare LLM
    if (dataToAnalyze.size() > 15) {
        dataToAnalyze = dataToAnalyze.stream()
            .limit(15)
            .collect(Collectors.toList());
    }
    
    // Costruisci il context dati
    StringBuilder dataContext = new StringBuilder();
    for (RenewableData d : dataToAnalyze) {
        dataContext.append(String.format(
            "- Data: %s, Prod: %.2f, Radiazione: %.1f, "
            + "Temp: %.1f, Vento: %.1f, Umidità: %d%%\n",
            d.getDateTime(),
            d.getSystemProduction(),
            d.getRadiation(),
            d.getAirTemperature(),
            d.getWindSpeed(),
            d.getRelativeAirHumidity()
        ));
    }
    
    // Delegama a LlmService
    return llmService.askAi(dataContext.toString());
}
    \end{lstlisting}
    
    \item \textbf{Prompt Inviato a Ollama:} Un esempio di prompt generato:
    \begin{lstlisting}[language=bash, caption=Prompt AI generato]
Sei un ingegnere energetico esperto. Analizza questi dati 
brevemente e dai un consiglio tecnico in italiano:
- Data: 2017-06-15, Prod: 2345.67, Radiazione: 850.0, 
  Temp: 28.5, Vento: 3.2, Umidità: 45%
- Data: 2017-06-16, Prod: 2198.34, Radiazione: 820.0, 
  Temp: 27.8, Vento: 4.1, Umidità: 52%
- Data: 2017-06-17, Prod: 2567.89, Radiazione: 880.0, 
  Temp: 29.1, Vento: 2.9, Umidità: 40%
    \end{lstlisting}
    
    \item \textbf{Richiesta HTTP a Ollama:} Il LlmService serializza il prompt in JSON:
    \begin{lstlisting}[language=json, caption=Body POST a http://localhost:11434/api/generate]
{
    "model": "llama3.2",
    "prompt": "Sei un ingegnere energetico esperto...",
    "stream": false
}
    \end{lstlisting}
    
    \item \textbf{Risposta Ollama:} Dopo alcuni secondi, Ollama ritorna:
    \begin{lstlisting}[language=json, caption=Risposta da Ollama]
{
    "response": "Analizzando i dati forniti, osservo una produzione 
        stabile con un picco il 17 giugno. Le condizioni meteo sono 
        favorevoli: radiazione elevata e temperature moderate. 
        La velocità del vento di 4.1 m/s nel 16 giugno ha contribuito 
        al leggero calo di produzione rispetto al 15. Consiglio: 
        effettuare pulizia dei moduli per optimizzare l'irraggiamento 
        in scenari con foschia.",
    "created_at": "2026-03-05T10:30:00Z",
    "done": true
}
    \end{lstlisting}
    
    \item \textbf{Risposta al Client:} Il backend estrae il campo \texttt{response} e lo ritorna al client:
    \begin{lstlisting}[language=bash, caption=Risposta finale al client CLI]
ANALISI INTELLIGENTE (Ollama - Llama3.2):

Analizzando i dati forniti, osservo una produzione stabile 
con un picco il 17 giugno. Le condizioni meteo sono favorevoli:
radiazione elevata e temperature moderate. La velocità del vento 
di 4.1 m/s nel 16 giugno ha contribuito al leggero calo di produzione 
rispetto al 15. Consiglio: effettuare pulizia dei moduli per optimizzare 
l'irraggiamento in scenari con foschia.
    \end{lstlisting}
\end{enumerate}

\subsection{Configurazione Ollama}
Per poter utilizzare questa funzionalità, è necessario:
\begin{enumerate}
    \item Installare Ollama da \texttt{https://ollama.ai}
    \item Avviare il server: \texttt{ollama serve}
    \item Scarica il modello: \texttt{ollama pull llama3.2}
    \item Verificare che sia raggiungibile su \texttt{http://localhost:11434}
\end{enumerate}

Modelli alternativi compatibili: \texttt{mistral}, \texttt{neural-chat}, \texttt{codellama}.
...
Tramite l'opzione 9 del Client, il sistema invia un sottoinsieme di dati critici a una LLM locale.
\begin{itemize}
...
Durante la fase di test e validazione del sistema, sono stati identificati e risolti diversi problemi critici relativi alla gestione dei dati, alla sicurezza e all'integrazione con sistemi LLM esterni.

\section{Problema 1: Parsing JSON Non Robusto nel Frontend}

\subsection{Sintomo}
Quando il frontend JavaScript tentava di parsificare risposte non-JSON dal server (ad es. fallback con emoji o testo semplice), il parser sollevava eccezioni tipo:
\begin{lstlisting}[language=bash, caption=Errore osservato nel browser]
Errore richiesta: Unexpected token '⚠', "⚠️ MODALIT"... is not valid JSON
Errore richiesta: Unexpected token 'A', "ANALISI VE"... is not valid JSON
\end{lstlisting}

\subsection{Causa Radice}
Il servizio \texttt{LlmService} ritornava un fallback contenente emoji quando Ollama non era disponibile:
\begin{lstlisting}[language=bash, caption=Fallback non-JSON]
⚠️ MODALITÀ OFFLINE (Ollama non disponibile)
Analisi sintetica dei dati forniti...
\end{lstlisting}

Il frontend, vedendo l'header \texttt{Content-Type: application/json}, tentava il parsing senza prima verificare se il contenuto era effettivamente JSON valido.

\subsection{Soluzione Implementata}
Aggiornamento di `\texttt{app.js}` (frontend) con try-catch robusto:
\begin{lstlisting}[language=JavaScript, caption=Parsing JSON robusto]
if (contentType.includes("application/json")) {
    try {
        return JSON.parse(text);
    } catch {
        return text;  // Ritorna il testo se non parsabile
    }
}
...
try {
    ...
    return text;  // Fallback sempre disponibile
}
\end{lstlisting}

\subsection{Test di Validazione}
Il fix è stato testato con:
\begin{itemize}
    \item Risposte contenenti emoji (✅ Passato).
    \item Risposte contenenti solo testo (✅ Passato).
    \item Risposte JSON valide (✅ Passato).
    \item Esecuzione dei test Maven: \textbf{BUILD SUCCESS}.
\end{itemize}

\section{Problema 2: Bug di Formattazione in Financial Report}

\subsection{Sintomo}
L'endpoint \texttt{/api/energy/financial-report} causava un'eccezione:
\begin{lstlisting}[language=bash, caption=Errore runtime osservato]
Exception: java.util.MissingFormatArgumentException: 
Format specifier '%.2f' not present in arguments
\end{lstlisting}

\subsection{Causa Radice}
Nel metodo \texttt{getFinancialReport()} di \texttt{EnergyController.java}, il formato string conteneva 4 placeholder (\texttt{\%.2f}) ma l'argomento finale era concatenato nella stringa stessa:
\begin{lstlisting}[language=Java, caption=Codice errato]
return String.format(
    "REPORT ECONOMICO & ECOLOGICO:\n" +
    "- Energia Totale Prodotta: %.2f kWh\n" +
    "- Valore Economico Generato: %.2f € (a %.2f €/kWh)\n" +
    "- CO2 Risparmiata: %.2f kg\n",
    totalKwh, totalValue, energyPrice, co2Saved  // ← Manca il quarto argomento nella stringa
);
\end{lstlisting}

\subsection{Soluzione}
Correzione della sintassi di \texttt{String.format()}:
\begin{lstlisting}[language=Java, caption=Codice corretto]
return String.format(
    "REPORT ECONOMICO & ECOLOGICO:\n" +
    "- Energia Totale Prodotta: %.2f kWh\n" +
    "- Valore Economico Generato: %.2f € (a %.2f €/kWh)\n" +
...
Il fix è stato verificato tramite:
\begin{itemize}
    \item Test endpoint diretto: \textbf{HTTP 200 OK}.
    \item Esecuzione test suite Maven: \textbf{BUILD SUCCESS}.
    \item Verifica risposta contiene valori numerici corretti.
\end{itemize}

\section{Problema 3: Integrazione Lenta con Ollama}

\subsection{Sintomo}
Le richieste all'endpoint \texttt{/api/energy/smart-analysis} richiedevano 30-60 secondi per completarsi (inferenza su CPU).

\subsection{Causa Radice}
\begin{itemize}
    \item Modello \textbf{llama3.2} (2.0 GB) processato su CPU (senza GPU).
    \item Timeout insufficiente (default di Spring RestTemplate: 30 secondi).
    \item Il modello non era configurato in Ollama.
\end{itemize}

\subsection{Soluzione Multi-Livello}
\subsubsection{1. Configurazione Timeout Adeguato}
Aggiornamento di \texttt{LlmService.java}:
\begin{lstlisting}[language=Java, caption=RestTemplate con timeout 5 minuti]
RestTemplate restTemplate = new RestTemplateBuilder()
    .setConnectTimeout(java.time.Duration.ofSeconds(10))
    .setReadTimeout(java.time.Duration.ofMinutes(5))  // 5 minuti
    .build();
\end{lstlisting}

\subsubsection{2. Fallback Multi-Model}
Implementazione di fallback automatico tra modelli:
\begin{lstlisting}[language=Java, caption=Modelli in ordine di velocità]
...
// llama3.2:        ~1-2m,   massima qualità
private static final String[] MODEL_NAMES = 
    {"neural-chat", "tinyllama", "phi", "llama3.2"};

for (String model : MODEL_NAMES) {
    try {
        // Tenta con il modello corrente
        Map<String, Object> response = 
            restTemplate.postForObject(OLLAMA_URL, entity, Map.class);
        if (response != null && response.containsKey("response")) {
            return response.get("response").toString();
        }
    } catch (Exception e) {
        // Prova il modello successivo
        System.err.println("LLM error with model " + model);
    }
}
\end{lstlisting}

\subsubsection{3. Indicatore di Caricamento nel Frontend}
Aggiornamento di \texttt{app.js}:
\begin{lstlisting}[language=JavaScript, caption=Messaggione di caricamento]
if (action === "smart") {
    renderOutput("⏳ Analisi IA in corso... " +
                 "(può richiedere 30-60 secondi)");
}
await runAction(action);
\end{lstlisting}

\subsubsection{4. Download Modello}
Scaricamento di \textbf{neural-chat} (4.1 GB) tramite:
\begin{lstlisting}[language=bash, caption=Comando per scaricare il modello]
docker exec -it helios-ollama /usr/bin/ollama pull neural-chat
\end{lstlisting}

\subsection{Risultati Ottenuti}
Dopo l'ottimizzazione:
\begin{itemize}
    \item \textbf{Tempo di risposta:} Ridotto da timeout a 30-60 secondi per neural-chat.
    \item \textbf{Robustezza:} Fallback automatico se un modello non è disponibile.
    \item \textbf{UX:} Messaggi informativi durante l'attesa.
    \item \textbf{Qualità:} Analisi semantiche complete e coerenti in italiano.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Analisi intelligente tramite LLM locale (Ollama + neural-chat). Integrazione dell'Adapter Pattern per IA generativa.}
...
\begin{table}[H]
    ...
    \begin{tabular}{ll}
        ...
        Frontend & JavaScript SPA (HTML/CSS/JS) \\
        LLM Integration & Ollama (neural-chat + fallback) \\
...
L'integrazione di pattern consolidati come Facade e DTO ha garantito performance e manutenibilità, mentre l'adozione di tecnologie emergenti come le LLM locali ha aggiunto un livello di accessibilità e intelligenza ai dati non comune nei sistemi SCADA tradizionali.

I risultati ottenuti sul dataset di test confermano la validità economica e tecnica della soluzione per contesti industriali. La fase di testing ha inoltre rivelato l'importanza della robustezza (error handling, timeout management) e dell'ottimizzazione (multi-model fallback, frontend caching) nella costruzione di sistemi distribuiti affidabili.
